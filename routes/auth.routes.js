...
router.post(
	...
	[
		...
		check('password', 'Минимльная длинна пароля 6 символов')
			.isLength({min: 6 })
	],
	async (req, res) => {
	try {
...
router.post(
	'/login',
	[
		check('email', 'Введите орректный email').normalizeEmail().isEmail(),
	...
	async (req, res) => {
					try {
						...
						const isNatch = await bcrypt.compare(password, user.password)
